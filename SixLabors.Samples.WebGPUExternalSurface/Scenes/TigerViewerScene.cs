...
namespace SixLabors.Samples.WebGPUExternalSurfaceDemo.Scenes;

/// <summary>
/// Loads the Ghostscript Tiger SVG via the shared <see cref="SvgHelper"/> and renders it with
...
internal sealed class TigerViewerScene : RenderScene
{
    ...
    public TigerViewerScene()
    {
        ...
        List<SvgHelper.SvgElement> parsed = SvgHelper.ParseSvg(svgPath);
        List<(IPath Path, SolidBrush? Fill, SolidPen? Stroke)> built = [];
        foreach ((IPath p, SolidBrush? f, SolidPen? s) in SvgHelper.BuildImageSharpElements(parsed, scale: 1f))
        {
            built.Add((p, f, s));
        }
