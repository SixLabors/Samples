...
namespace SixLabors.Samples.WebGPUExternalSurfaceDemo;

/// <summary>
/// Shared SVG parsing and ImageSharp element construction for SVG rendering samples.
/// </summary>
internal static class SvgHelper
{
    ...
    /// </summary>
    public static List<SvgElement> ParseSvg(string filePath)
    {
        ...
        foreach (XElement pathEl in doc.Descendants(ns + "path"))
        {
            ...
            else if (Color.TryParse(fillStr, out Color parsed))
            {
                fill = parsed;
            }
            else
            ...
            // Parse stroke.
            Color stroke = ParseColor(ResolveInheritedPresentationValue(pathEl, "stroke"));
    ...
    /// </summary>
    public static List<(IPath Path, SolidBrush? Fill, SolidPen? Stroke)> BuildImageSharpElements(
        List<SvgElement> elements,
        float scale)
    {
        List<(IPath, SolidBrush?, SolidPen?)> result = [];
        foreach (SvgElement el in elements)
        {
            ...
            SolidBrush? fill = fillPixel.A > 0
                ? new SolidBrush(el.Fill)
                : null;
    ...
    private static Color ParseColor(string? value)
    {
        ...
        return Color.TryParse(value, out Color color) ? color : Color.Transparent;
    }
